...
因為正常程式碼執行是由上往下照順序執行。但實際情況並不是這樣

1. 因應各種情況、各種判斷，所需要執行的程式碼片段可能有所不同，這時候用 func 來包裝程式碼，是常用的一種做法。

2. 再來就是某些程式碼可能會重複，且不斷地執行，那每次使用就每次寫重複的程式碼，是一件非常愚蠢的行為。將需要重複執行的程式碼用 func 包裝起來，也是常用作法。
